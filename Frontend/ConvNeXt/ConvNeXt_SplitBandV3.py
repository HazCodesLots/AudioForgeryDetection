...
# =============================================================================
# ConvNeXt-SplitBand V3: Hybrid Margin BCE + 16kHz Revert
# =============================================================================
# Changes vs V2 (ASL):
...
# ---------------------------------------------------------------------------
# 3. LSE Pooling + SplitBandPoolingV3
# ---------------------------------------------------------------------------
...
class SplitBandPoolingV3(nn.Module):
...
class ConvNeXtTagger(nn.Module):
    def __init__(self, convnext_params, pooling_params, mlp_params):
        ...
        self.pooling  = SplitBandPoolingV3(**pooling_params)
...
def train_model(model, train_loader, val_loader, device, num_epochs=100,
                save_path='results/ConvNeXt_SplitBandV3/best_model.pth',
                resume=False, checkpoint_path='results/ConvNeXt_SplitBandV3/checkpoint.pth',
                metrics_path='results/ConvNeXt_SplitBandV3/metrics.json'):
    ...
    history = {'train_loss':[], 'val_loss':[], 'val_mAP':[], 'grad_norm':[], 'update_ratio':[], 'lse_beta':[]}
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        maps = [average_precision_score(targets[:,i], preds[:,i]) for i in range(23) if targets[:,i].sum()>0]
        cur_map = np.mean(maps)
        ...
        print(f" mAP: {cur_map:.4f} | β: {history['lse_beta'][-1]:.3f} | LR: {optimizer.param_groups[0]['lr']:.6f}")
        ...
        torch.save({'epoch':epoch, 'model':model.state_dict(), 'optimizer':optimizer.state_dict(),
                    'scheduler':scheduler.state_dict(), 'best_map':best_map, 'history':history}, checkpoint_path)
        with open(metrics_path, 'w') as f: json.dump(history, f)
