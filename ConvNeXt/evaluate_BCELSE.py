...
# Modular Import: Use the classes directly from the BCELSE model script
from ConvNeXt_SplitbandBCELSE import ConvNeXtTagger, EnhancedAudioFrontend, SONYCUSTDataset, pad_truncate_collate
...
COARSE_LABELS = [
    '1_engine', '2_machinery-impact', '3_non-machinery-impact', '4_powered-saw',
    '5_alert-signal', '6_music', '7_human-voice', '8_dog'
]
...
def run_evaluation(dataset_path, weights_path, split='test', batch_size=16):
    ...
    print(f"Device: {device} | Split: {split} | Model: BCELSE")
    ...
    # Standardized params (to be updated if BCELSE is scaled up)
    ...
    print(f"\nResults for {split} split (BCELSE):")
    print(f"  Fine Macro-AUPRC:   {macro_auprc_fine:.4f}")
    print(f"  Coarse Macro-AUPRC: {macro_auprc_coarse:.4f}")

    results = {
        'model': 'BCELSE',
        'split': split,
        'fine_macro_auprc': macro_auprc_fine,
        'coarse_macro_auprc': macro_auprc_coarse,
        'per_class_fine': {label: float(ap) for label, ap in zip(FINE_LABELS, fine_aps)}
    }
    
    out_path = os.path.join(os.path.dirname(weights_path), f"eval_bcelse_{split}.json")
